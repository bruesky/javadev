...
public abstract class TechSupportSpecialist
{
    ...
    {
        // TODO
        if (isAvailable()&&isResponsibleFor(request.getCustomer())&&isQualifiedToHandle(request.getSubject())){
            setAvailable(false);
            request.setStatus(ServiceRequest.STATUS_ASSIGNED);
            resolveRequest(request);
            request.setStatus(ServiceRequest.STATUS_CLOSED);
            setAvailable(true);
        }else {
            if (this.next!=null)
                this.next.handleRequest(request);
            else
                request.setResponse("Please try again later.");
        }
    }
