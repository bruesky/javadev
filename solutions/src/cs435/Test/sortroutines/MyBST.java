...
public class MyBST {
    ...
    public void bstSortHelp(int[]arr,int index){
        if (root != null)
            printTreeHelp(root,arr,index);
    }

    private void printTreeHelp(Node t,int[]arr,int index) {
        if (t != null) {
            printTreeHelp(t.left,arr,index+1);
            arr[index] = t.element;
            printTreeHelp(t.right,arr,index+2);
        }
    }
